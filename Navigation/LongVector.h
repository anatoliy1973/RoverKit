...
public:
	...
	{
		return new LongVector(Multiply(x.m_x, y), Multiply(x.m_y, y), Multiply(x.m_z, y));
	}
	...
	{
		return Multiply(x.m_x, y.m_y) + Multiply(x.m_y, y.m_y) + Multiply(x.m_z, y.m_z);
	}
	
	friend LongVector* operator ^ (const LongVector& u, const LongVector& v)
	{
		return new LongVector(
			Multiply(u.m_y, v.m_z) - Multiply(u.m_z, v.m_y),
			Multiply(u.m_z, v.m_x) - Multiply(u.m_x, v.m_z),
			Multiply(u.m_x, v.m_y) - Multiply(u.m_y, v.m_x));
	}
